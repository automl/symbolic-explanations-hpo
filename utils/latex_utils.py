...
def generate_result_table(df, stddev_df, stddev: bool = False, decimal_places: int = 3,
    ...
    for k in range(len(df.index)):
        df.iloc[k] = df.iloc[k].apply(
            lambda data: bold_extreme_values(data, best=df.iloc[k].min(),
                                             second_best=np.partition(df.iloc[k].array.to_numpy(), 1)[1],
                                             decimal_places=decimal_places))

    for k in range(len(df_decimal_format.index)):
        df_decimal_format.iloc[k] = df_decimal_format.iloc[k].apply(
    ...
    df["Hyperparameters"] = df["Hyperparameters"].str.replace('_', '\_')
